...
class AsokaTypeOld:
    ...
    def tableExist(self):
        if not self.__database_is_ok__():
            Console.warning("Type.checkTable(): обращение к SQL с некорректными параметрами")
            return False
    ...
    def createTable(self):
        if not self.__database_is_ok__():
            Console.warning("Type.createTable(): обращение к SQL с некорректными параметрами")
            return
    ...
    def load(self, id = asoka.nullInt):
        ...
        if len(data) != 0:
            ...
            Console.error("Type.load(): пустой результат SQL запроса")

    def loadField(self, name):
        if not self.__database_is_ok__():
            Console.warning("Type.loadField(): обращение к SQL с некорректными параметрами")
            return
        ...
        try:
            ...
            Console.error("Type.loadField(): ошибка получения данных")
            return None
    ...
    def insert(self):
        if not self.__database_is_ok__():
            Console.warning("Type.insert(): обращение к SQL с некорректными параметрами")
            return
        table = self.database.get('table')
        fields = ""
        args = ""
        ...
        if db_type == 'sqlite':
            ...
            try:
                ...
                Console.error(f"Type.insert(): {table}: Не удалось получить id")

    def update(self):
        if not self.__database_is_ok__():
            Console.warning("Type.update(): обращение к SQL с некорректными параметрами")
            return
        table = self.database.get('table')
        id = self.get('id')
        args = ""
        ...
        for key in structure.keys():
            ...
            if field_type is None:
                Console.error('Type.update(): поле класса имеет тип None')
                return
    ...
    def remove(self):
        if not self.__database_is_ok__():
            Console.warning("Type.remove(): обращение к SQL с некорректными параметрами")
            return
        if self.getId() == asoka.nullInt:
            Console.warning("Type.remove(): невозможно выполнить с неактуальным id")
            return
