#include "DefaultTypes/Assistent/Linguistics/morpheme_analyzer.h"
...
StemmerPorterResult StemmerPorter::reduce(astring str)
{
    StemmerPorter stemmer;
    StemmerPorterResult result;
    result.orginal = str;
    result.clipped = stemmer.get(str.toStdString());
    result.deleted = str.remove(result.clipped);
    return result;
}
